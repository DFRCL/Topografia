...
def estacion_inicio():
    ...
    Y = input('Ingrese Y: ')
    Z = input('Ingrese Z: ')
    Punto_inicio = Punto(Nombre, X, Y, Z)
...
def Plot_to_dxf(filename):
    ...
    for poligono in Poligono.all:
        ...
        p = drawing.modelspace.add(dxf.polyline(points = obtener_vertices(poligono), linetype=linetype(poligono), layer=poligono.Nombre, color=1))
        p.close()
...
if __name__ == '__main__':
    ...
    Punto('1', 0, 0, 0)
    LibretaL = transform_datatypes(askopenfilename())
    for row in LibretaL:
        Radiacion(**row)
    crear_poligonos()
    #for i in Poligono.all:
     #   i.add_points()
    for i in Observacion.all:
        Totales(i)
    Plot_to_dxf(asksaveasfilename(defaultextension='.dxf'))
